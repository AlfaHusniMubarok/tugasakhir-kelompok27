...
const DetailOuter = createContext();
...
export default function Outer() {
    ...
    return (
        <div style={{ marginTop: 20 }}>
            ...
            <center>
            <h1 className="text-3xl font-extrabold tracking-tight text-gray-900" style={{fontFamily:"Roboto", letterSpacing:"1px"}}>Daftar Outer</h1> 
            </center>
            ...
            <DetailOuter.Provider value={{ judul: judul, deskripsi: deskripsi }}>
                <div>
                    <Modal
                        open={open}
                        onClose={handleClose}
                        aria-labelledby="modal-modal-outer"
                        aria-describedby="modal-modal-deskripsi"
                    >
                        <Detail />
                    </Modal>
                </div>
            </DetailOuter.Provider>
        </div >
    );
}

function Detail() {
    const info = useContext(DetailOuter);
    return (
        <Box sx={style}>
            <center>
            <Typography id="modal-modal-outer" variant="h6" component="h2">
                {info.judul}
            </Typography>
            </center>
            <Typography id="modal-modal-deskripsi" sx={{ mt: 1 }}>
                Merk: {info.deskripsi.merk}
            </Typography>
            <Typography id="modal-modal-deskripsi" sx={{ mt: 1 }}>
